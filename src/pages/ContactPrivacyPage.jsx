    ...
    <div style={{ position: 'relative', zIndex: 1, maxWidth: '600px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      {sent ? (
        ...
        <div style={{ background: '#0f1420', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '36px' }}>
          <label style={{ display: 'block', color: '#e8edf5', fontSize: '0.85rem', marginBottom: '6px', fontWeight: 500 }}>Your Name</label>
          <input style={inputStyle} placeholder="John Doe" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            onFocus={e => e.target.style.borderColor = 'rgba(0,240,160,0.4)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.07)'} />

          <label style={{ display: 'block', color: '#e8edf5', fontSize: '0.85rem', marginBottom: '6px', fontWeight: 500 }}>Email</label>
          <input style={inputStyle} placeholder="you@email.com" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
            onFocus={e => e.target.style.borderColor = 'rgba(0,240,160,0.4)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.07)'} />

          <label style={{ display: 'block', color: '#e8edf5', fontSize: '0.85rem', marginBottom: '6px', fontWeight: 500 }}>Message</label>
          <textarea style={{ ...inputStyle, minHeight: '120px', resize: 'vertical' }} placeholder="Your message..."
            value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
            onFocus={e => e.target.style.borderColor = 'rgba(0,240,160,0.4)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.07)'} />

          <button onClick={() => form.name && form.email && setSent(true)} style={{
...
export function PrivacyPage() {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, maxWidth: '700px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      {sections.map((s, i) => (
        <div key={i} style={{ marginBottom: '32px', paddingBottom: '32px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
